...
const TAB_CONFIG = {
    ...
    'connection': {
        ...
        loadFunction: 'loadConnectionsPaginated',
        containerSelector: '#connection-tab .pagination-container'
    },
    'default': {
        state: window.state,
...
const TAB_CONFIG = {
    ...
    'connections': { // Fixed: was 'connection', now 'connections'
        state: window.connectionState,
        loadFunction: 'loadConnectionsPaginated',
        containerSelector: '#connection-tab .pagination-container'
    },
...
function gotoPage(p, tabType = 'default') {
    ...
    if (next !== currentState.page) {
        ...
        switch (tabType) {
            ...
            case 'connections': // FIXED: This was the problematic case
                if (typeof window.loadConnectionsPaginated === 'function') {
                    window.loadConnectionsPaginated(next, currentState.size);
                } else if (typeof window.connectionPaginated !== 'undefined' && window.connectionPaginated.loadDataPaginated) {
                    window.connectionPaginated.loadDataPaginated(next, currentState.size);
                } else {
                    console.error('Connection pagination function not found');
                }
                break;
...
function getCurrentActiveTab() {
    ...
    if (activeTab) {
        ...
        if (tabId.includes('announcement') || tabId.includes('broadcast')) {
            ...
            return 'assignments';
        } else if (tabId.includes('connection')) {
            return 'connections';
        }
    }
    return 'default';
...
// Usage examples:
// For announcements:
// renderPaginationFooters("#broadcast-tab .pagination-container", window.announcementState, 'announcements');

// For assignments:
// renderPaginationFooters("#assignment-tab .pagination-container", window.assignmentState, 'assignments');

// For connections:
// renderPaginationFooters("#connection-tab .pagination-container", window.connectionState, 'connections');

// For default/other content:
// renderPaginationFooters(".pagination-container", window.state, 'default');